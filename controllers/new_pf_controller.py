...
class RobustPFController:
    ...
    def __init__(
        ...
        try:
            ...
            particles_is_integral = float(particles) == particle_count
        except (TypeError, ValueError, OverflowError) as exc:
            raise ValueError("Variable-K PF requires an integer particle count") from exc
        if isinstance(particles, bool) or not particles_is_integral or particle_count < 60:
    ...
    def recommend(self) -> ControllerAction:
        ...
        remaining = remaining_dose_ml(
            self.base_added_ml + self.acid_added_ml, self.max_total_dose_ml
        )
        ...
        max_candidate = min(MAX_ACTION_VOLUME_ML, remaining)
        candidate_volumes = np.asarray(
            [round(MIN_ACTION_VOLUME_ML * index, 2) for index in range(1, 1001)
             if round(MIN_ACTION_VOLUME_ML * index, 2) <= max_candidate + 1e-12],
            dtype=float,
        )
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "protocol": controller_protocol_metadata(
                ...
                persistent_overshoot_cap="enabled_pf_persistent_half-dose_threshold",
            ),
