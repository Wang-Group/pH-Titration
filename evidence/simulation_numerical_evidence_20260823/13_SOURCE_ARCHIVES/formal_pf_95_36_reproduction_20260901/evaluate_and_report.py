...
def teacher_episode(task, particles, seed):
    ...
    true_error = abs(true_ph - controller.target_ph)
    return {
        "task_seed": task.seed,
...
def plot_results(aggregate, ppo_root, output_dir):
    ...
    if curves:
        ...
        for seed in sorted({int(row["seed"]) for row in curves}):
            seed_curves[seed] = sorted(
                (row for row in curves if int(row["seed"]) == seed),
                key=lambda row: int(float(row["environment_steps"])),
            )
        colors = plt.get_cmap("tab10").colors
        for color, (seed, rows) in zip(colors, seed_curves.items()):
            ax.plot(
                [int(float(row["environment_steps"])) for row in rows],
        ...
        aligned_length = min(len(rows) for rows in seed_curves.values())
        checkpoints = np.asarray(
            [
                np.mean([int(float(rows[index]["environment_steps"])) for rows in seed_curves.values()])
        ...
        aligned = np.asarray(
            [
                [float(rows[index]["success_rate_percent"]) for index in range(aligned_length)]
                for rows in seed_curves.values()
            ],
            dtype=float,
        )
        means = np.mean(aligned, axis=0)
        sds = np.std(aligned, axis=0, ddof=1) if len(seed_curves) > 1 else np.zeros(aligned_length)
        ax.plot(checkpoints, means, color="#111111", linewidth=2.4, label="mean")
        ax.fill_between(checkpoints, means - sds, means + sds, color="#555555", alpha=0.13, label="mean +/- SD")
        ax.set_xlabel("Environment interactions")
        ax.set_ylabel("Validation success (%)")
        lower = min(100.0, float(np.min(aligned - sds)), float(np.min(aligned)))
        ax.set_ylim(max(0.0, math.floor((lower - 2.0) / 5.0) * 5.0), 100)
        ax.set_title("PPO validation learning curves")
