...
class PolicyEnvironment:
    """Direction-assisted environment in which the policy selects volume only.

    The external controller applies one fixed rule: add base when measured pH is
    below the target and acid otherwise. It does not mask actions after an
    overshoot, reduce the permitted volume, or switch titrant concentration.
    """
    ...
    def step(self, requested_volume_ml: float) -> None:
        if self.done:
            return
        requested = float(np.clip(requested_volume_ml, MIN_VOLUME_ML, MAX_VOLUME_ML))
        ...
        concentration = self.primary_titrant_m
