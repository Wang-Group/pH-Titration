...
try:  # Supports both ``import controllers`` and legacy direct execution.
...
class FixedKParticleFilter:
    def __init__(
        ...
        try:
            ...
            count_is_integral = float(particle_count) == count
        except (TypeError, ValueError, OverflowError) as exc:
            raise ValueError("particle_count must be an integer") from exc
        if isinstance(particle_count, bool) or not count_is_integral or count < 20:
        ...
        try:
            fixed_concentration = float(fixed_concentration_m)
        except (TypeError, ValueError) as exc:
            raise ValueError("fixed_concentration_m must be finite and positive") from exc
        if not np.isfinite(fixed_concentration) or fixed_concentration <= 0.0:
            raise ValueError("fixed_concentration_m must be finite and positive")
        self.particle_count = count
        self.pair_count = int(pair_count)
        self.infer_concentration = bool(infer_concentration)
        self.fixed_concentration_m = fixed_concentration
        self.rng = rng
    ...
    def update(
        ...
        if ess < ESS_THRESHOLD_FRACTION * self.particle_count:
            ...
            # Liu-West rejuvenation can propose implausible extreme pKa values.
            # Keep particles inside a broad numerical/chemical stability envelope
            # before the next charge-balance solve; this is regularization, not a
            # claim that every physical species has pKa in this interval.
...
class VariableKParticleFilter:
    ...
    def __init__(self, particle_count: int, rng: np.random.Generator) -> None:
        try:
            ...
            raise ValueError("particle_count must be an integer") from exc
        if isinstance(particle_count, bool) or not count_is_integral or count < 60:
